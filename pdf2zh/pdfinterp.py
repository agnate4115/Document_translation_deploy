...
class PDFPageInterpreterEx(PDFPageInterpreter):
    ...
    def __init__(
        ...
        # IMPORTANT: pdfminer initializes interpreter state (including `ncs`/`scs`)
        # in the base constructor. If we don't call it, some PDFs crash with:
        #   AttributeError: 'PDFPageInterpreterEx' object has no attribute 'ncs'
        super().__init__(rsrcmgr, device)
        # pdfminer versions vary: some expose `ncs/scs`, some don't.
        # Our operator handlers (`do_scn/do_SCN`) rely on them, so ensure
