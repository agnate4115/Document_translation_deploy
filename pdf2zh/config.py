        ...
        if not self._config_path.exists():
            if isInit:
                try:
                    ...
                    # If we can't create config directory, use in-memory config only
                    import logging
                    logger = logging.getLogger(__name__)
                    logger.warning(f"Could not create config directory {self._config_path.parent}: {e}. Using in-memory config only.")
                    self._config_data = {}  # Use in-memory config only
            else:
            ...
            try:
                ...
                import logging
                logger = logging.getLogger(__name__)
                logger.warning(f"Could not load config file {self._config_path}: {e}. Using in-memory config only.")
                self._config_data = {}
    ...
    def _load_config(self):
        ...
        with self._lock:  # 加锁确保线程安全
            try:
                ...
                # If we can't load config, use empty config
                import logging
                logger = logging.getLogger(__name__)
                logger.debug(f"Could not load config from {self._config_path}. Using empty config.")
                self._config_data = {}
    ...
    def _save_config(self):
        ...
        with self._lock:  # 加锁确保线程安全
            try:
                ...
                # If we can't save config, just continue with in-memory config
                import logging
                logger = logging.getLogger(__name__)
                logger.debug(f"Could not save config to {self._config_path}. Using in-memory config only.")
